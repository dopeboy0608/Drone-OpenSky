...
이슈에 착수할 때는(기획/그릴미로 상세화하고 이슈 등록까지 끝난 뒤) `issue-start` 스킬(`.claude/skills/issue-start/`)로 시작한다. 이 스킬이 이슈 내용을 읽고 아래 두 시나리오 중 하나를 추천하며, 최종 선택은 항상 사용자가 한다.

- **A. 기존 방식(바로 구현)**: 브랜치 생성 → 구현 → 테스트/포맷/린트 → (요청 시) 커밋/PR
- **B. spec-kit 경로**: `speckit-start` 스킬(`.claude/skills/speckit-start/`)이 `/speckit-specify` → `/speckit-plan` → `/speckit-tasks` → `/speckit-implement`를 단계별 확인을 받으며 진행

추천 판단 기준은 [ARCHITECTURE.md "spec-kit 적용 기준"](./docs/ARCHITECTURE.md#spec-kit-적용-기준) 참고. 두 경로 모두 이슈 라벨/브랜치 컨벤션(아래 GitHub 이슈 관리) 및 커밋 규칙(사용자 명시적 요청 시에만)을 동일하게 따른다.

두 스킬은 현재 이 프로젝트 전용이다(grill-me·spec-kit 의존성이 다른 프로젝트엔 없을 수 있음). 트라이얼 단계로, 이 프로젝트에서 계속 사용하며 판단 기준과 스킬 동작을 다듬은 뒤 사용자 전역(`~/.claude/skills/`)으로 분리할 예정이다.
...
디버깅, 원인 조사, 기술적 의사결정(예: 특정 방식이 왜 안 되는지 검토) 등 문제 해결 과정으로 판단되는 대화나 작업은 `docs/TROUBLE_SHOOTING.md`에 누적 기록한다. 일별 작업 히스토리 로그와 달리 이 파일은 git으로 추적한다 (동일한 문제를 겪을 수 있는 다른 사람에게 참고가 되는 공개 레퍼런스 성격).
